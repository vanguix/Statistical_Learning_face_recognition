...
for (metric in metrics) {
  ...
  legend("topright", legend = seq(min(distance_matrix), max(distance_matrix)-1, length.out = 11),fill = heat.colors(11), title = "Values", cex = 0.8)
}
```

The results in the heatmaps indicate that the Manhattan distance is the one that seems to give better results. The intra-class distances colored in dark red are the highest and in the heatmap they are easily distinguished. On the other hand, the inter-class distances are lower and are coloured in lighter colours.
