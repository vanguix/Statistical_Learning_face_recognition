...
```{r}
classifier <- function(df_train_data, df_test_data,PCAopt,k,thres,metric='manhattan', perc) {
  # INPUT
...
thresholds = seq(min(distance_matrix), max(distance_matrix), lenght.out = 11)
...
for (th in thresholds) {
  for (var in percentage_var) {
    for (k_v in k_values) {
      ...
      resultados_df <- bind_rows(resultados_df, data.frame(metric = "manhattan", threshold = th, percentage_var = var, k_value = k_v, accuracy1 = a[1], accuracy2 = a[2]))
    }
  }
}
```
