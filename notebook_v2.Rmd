...
First, we loaded the data and transformed each image to appropriate format. Later, we split the dataset for the hyperparameter tuning into two sets: train and test. For every individual in the training set there is one image in the test set. However, to develop a classifier that is able to distinguish between people that are in the dataset and people that are not, we have defined two different categories for the instances in the test set:
...
Then we have applied the corresponding dimensionality reduction techniques:

-   For Part A, Principal Component Analysis has been used to reduce the dimensionality of the data.

-   For part B, Fisher Discriminant Analysis has been used. As this algorithm is too complicated to implement in high dimensionality data, PCA was also applied to the data previously.

For both case A and case B, hyperparameter tuning has been used to determine the optimum values for the following hyperparameters:
...
First, as a first approach to decide the metric and the threshold, we have plotted the heatmaps for each different metric. This helps decide which are the best metrics to distinguish the different individuals, also allows to estimate the values that seem reasonable for the thresholds.

Then, once this previous study is already done and the metric is determined, a grid search is performed to iterate over the different combinations of the other hyperparameters and compute the accuracy each time. Finally, the combination that corresponds to the highest accuracy will be chosen for both the case of PCA and the Fisher. For part A, the raw data has also been used with the classifier, to determine the differences in execution time and performance with that raw data.
...
In this first part, the functions that will be used later will be defined and explained. Some functions are required for both the Part A and Part B (read_images, transform_data, split_train_test)
...
accuracy_old <- function(prediction, target, test_type){
  # INPUT
...
For the euclidean metric, a grid search will be performed, iterating over the
...
#### 2.2.1. Grid search for manhattan metric
...
Elegimos el threshold según el heatmap sin PCA, teniendo en cuenta el mejor threshold que nos ha salido en el hyperparameter tuning .

k es 3 porque si

euclidean igual

percentage of variance hay que meterlo porque la función classifier lo pide, pero al no hacer PCA no sirve pa na en verdad

```{r}
resultado <- classifier(train_matrix, test_matrix, 0, k = 3, thres = 18.36, metric = 'euclidean', 0.9, df_train_data$target)
acc <- accuracy(resultado, df_test_data$target, df_test_data$split)
cat('Accuracy belong= ', acc$accuracy_belong, '\n')
cat('Accuracy total= ', acc$accuracy_target, '\n')
