...
The classifier will be based on the K Nearest Neighbors algorithm. It will compute the distances between each picture and select the K closest images, that is, the K most similar images. Based on these images and knowing the individuals they correspond to, the category of the new image will be the most common category from these 'closest neighbors'.

Due to the high dimensionality of the data images, it is interesting to consider dimensionality reduction techniques. For this project, Principal Component Analysis and Fisher Discriminant Analysis are considered.
...
-   The first part uses the PCA and KNN for classifying the images

-   The second part uses the Fisher Discriminant Analysis and KNN for classifying. Here, it was also required to include the PCA to transform the data before using the Fisher Discriminant Analysis due to the large size of the data matrices.

For both cases, the best classifier possible has to be retrieved, for that reason, hyperparameter tuning was required. The methodology followed on each section is similar.

First, the required functions have been created (read the images, classification, etc.). The first step is to load the data and transform each image to the appropriate format.

Second, for hyperparameter tuning it is required to divide the available data into train and test set. This separation has been made taking into account two considerations: for every individual in the training set there are 5 images in the training set and 1 in the test set. Also, two individuals have not been included in the training set and all their images are only in the test set. This procedure aims to develop a classifier that is able to distinguish between people that are in the dataset and people that are not. So the test set is divided into two categories:
...
classifier <- function(train_matrix, test_matrix,PCAopt,k,thres,metric='manhattan', perc, target) {
  # INPUT
  # train_matrix - the data (without the label) of the training set (it is a matrix)
  # test_matrix - the data (without the label) of the test set (it is a matrix)
  ...
  # metric - type of metric for KNN
  # perc - percentage of variance of PCA
  # target - label values of the train data
  ...
  if (PCAopt == TRUE) {
    #1. Apply the PCA function only to the training
...
-   **accuracy**: this function computes the accuracy when you specify a prediction and a target.

```{r}
accuracy <- function(prediction, target){
  # INPUT
...
However, we will evaluate both distance types (Euclidean and Manhattan) to be sure to obtain the better results.
...
for (th in thresholds_euclidean) {
  ...
  for (var in percentage_var) {
    ...
    for (k_v in k_values) {
      ...
      acc <- accuracy(resultado, df_test_data$target)
...
#### 2.2.3. Fine tuning threshold and percentage of variance for euclidean distance
...
### 3.1. Tuning the threshold for raw data
...
With these results, the optimum hyperparameters for the raw data model are the following. The percentage of variance is not used, as the model is not applying PCA.

| **metric** | **threshold** | **perc_var** | **k_value** | **accuracy** |
|:-----------|:--------------|:-------------|:------------|:-------------|
| euclidean  | 31.67         | 0.9          | 3           | 0.978        |

## 4. Training of the final PCA model

The final model will use the PCA dimensionality reduction because it obtains an accuracy of 1.00 while being much faster. Also, it will use the optimum hyperparameters that have been already calculated in the previous sections.
...
The transformation matrix P, the transformed train data, the means of the columns, the percentage of variance explained D, the target of the training data and the KNN function will be stored in a Rdata file. This saves time from reading and transforming the training data each time.
...
fisher_lda <- function(data, labels, perc_fish, perc_pca=0.9) {
  ...
  # perc_fish -> percentage of variance explained wanted to retain for FDA
  # perc_pca -> percentage of variance explained wanted to retain for PCA
...
classifier_fisher <- function(train_matrix, test_matrix,k,thres,metric='manhattan', perc, target) {
  ...
  # thres - threshold value to determine whether a new image belongs to the dataset or not
  # metric - type of metric for KNN
  # perc - percentage of variance explained wanted to retain for FDA
  # target - vector of labels
  ...
  #Apply the Fisher Discriminant Analysis
...
-   the **percentage of variance** retained by the Fisher Discriminant Analysis.

-   the **number of neighbors** to use with the KNN.

The problem is similar to the previous one, but in this case, Fisher Discriminant Analysis will be used.

### 2.1. Heatmaps with Fisher

To apply the Fisher, a percentage of variance retained needs to be specified. As this is just a first approach for the hyperparameter tuning, a value that seems reasonable will be chosen. This value is a 95% of variance explained.
...
for (metric in metrics) {
  ...
  # Use the values of the distances to include the histogram
  legend("topright", legend = seq(min(distance_matrix), max(distance_matrix), length.out = 11),fill = heat.colors(11), title = "Values", cex = 0.8)
}
```
...
Now this previous analysis has been made, the remaining hyperparameters will be tuned. As the previous heatmaps show, the best distance metric is the Euclidean distance, so the model will use this metric. In this case, we do not evaluate again Manhattan, as previous results from PCA show already that Euclidean performed better.

The data partition that has been previously used for hyperparameter tuning with PCA, will be used now for hyperparameter tuning with Fisher. The model will be trained using the train partition and then the future performance will be evaluated with the test partition.
...
Now that the hyperparameter tuning has been made, it is time to train the final Fisher model. For this last stage, the whole dataset will be used in order to use all the available information and obtain a model as accurate as possible.
...
As a final step, one of the two classifiers must be chosen for deployment. As both has achieved the best accuracy, we could choose indistinctly any of them. However, our final option is the Fisher Classifier as it attempts to find the vector that maximizes the separation between classes of the projected data. However, PCA projects the data without any predefined objective more than dimensionality reduction. For a classification problem as ours, Fisher is a more adequate approach.
